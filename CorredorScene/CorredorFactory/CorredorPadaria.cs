...
{
    public class CorredorPadaria : MonoBehaviour, ICorredor
    {
        public void PrintarTitulo(TMP_Text componenteTitulo)
        {
            componenteTitulo.text = "Padaria";
        }

        public void RenderizarProdutos(GameObject componentePrateleira)
        {
            var products = ProductRepositorySingleton.GetInstance().GetBakeryProducts();
